...
class Backup {
  constructor(config) {
    ...
    this.mergedFiles = config.mergedFiles;
  }
      ...
      .then((allRecords) =>
        Promise.all(
          Object.keys(allRecords).map((key) =>
            this.dbRecord.backup(allRecords[key], true)
          )
        )
      )
      .catch((err) => {
  ...
  async full() {
    return await this.dbInstanceData.retrieve(this.mergedFiles).catch((err) => {
      throw err;
    });
  }
}
