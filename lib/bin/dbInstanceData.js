...
class DbInstanceData {
  ...
  async retrieve(getMerged) {
    return await this.getTableKeys()
      .then(async (keys) => {
        if (getMerged == true) {
          await this.exportData(keys).then(async (records) => {
            await this.dbRecord.backupChanges(records).catch((err) => {
              throw err;
            });
          });
        } else {
