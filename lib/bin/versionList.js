...
class VersionList {
  constructor(config, versionReaderFunc) {
    ...
    this.VersionReaderFunc =
      versionReaderFunc === undefined ? this.getAllVersions : versionReaderFunc;
  }

  async getVersions() {
    return await this.VersionReaderFunc()
      .then((allVersionList) => {
        // return this.versionListCreatedBeforeGivenTime(allVersionList)
        //   .then((versionList) => {
        //     return this.versionListCreatedAfterGivenTime(
        //       versionList,
        //       allVersionList
        //     );
        //   })
        ...
        //   })
        //   .then((versionList) => {
        return this.beautifyVersionList(allVersionList);
        //   })
        //   .catch((err) => {
        //     throw err;
        //   });
  ...
  //         if (
  //           !(data.Key in newData) ||
  //           new Date(newData[data.Key].LastModified) -
  ...
  //         let diff =
  //           this.RestoreTime === undefined
  //             ? -1
  //             : this.RestoreTime - new Date(data.LastModified);
  ...
  //         let existingVersions = filterObject.call(
  //           this,
  //           allObjectVersions,
  //           (version) => {
  ...
  async beautifyVersionList(versionList) {
    return new Promise((resolve) => {
      let newData = {};
