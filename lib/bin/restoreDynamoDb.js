...
class RestoreDynamoDb {
  ...
  async s3ToDynamoDb(versionList, merged) {
    return await this.getDbTableKeys()
      .then((keys) => {
        let promises = [];
        Object.keys(versionList).forEach(async (key, index) => {
          promises.push(
            await this.processVersion(versionList[key], keys, merged)
          );
        });
  ...
  async processVersion(version, keys, merged) {
    return await this.retrieveFromS3(version)
      .then(async (data) => {
        if (merged == true) {
          data[1].forEach(async (file) => {
            return await this.pushToDynamoDb(data[0], JSON.parse(file), keys);
          });
        } else {
